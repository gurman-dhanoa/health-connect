...
import { clearAnalyserError } from "../../../store/Analyser/Analyser";
import { analyserRegister } from "../../../store/Analyser/AnalyserAction";

const AnalyserRegister = () => {
  ...
  const submitForm = (data) => {
    ...
    dispatch(analyserRegister(data));
  };
  const { error, isAuthenticatedAnalyser } = useSelector((state) => state.user);
  ...
  useEffect(() => {
    if (error) {
      ...
      dispatch(clearAnalyserError());
    }
    if (isAuthenticatedAnalyser) {
      toast({
        title: "Success",
      ...
      navigate("/analyser/profile");
    }
  }, [dispatch, navigate, error, isAuthenticatedAnalyser, toast]);
  return (
    <VStack className="loginBgStyle" align={"flex-start"} m={"auto"}>
      <Box margin={"50px auto"}>
        <Heading color="black">Analyser Register</Heading>
...
export default AnalyserRegister;